...
class StressNg:
    ...
    def run(self):
        ...
        if len(self.stressors) == 1:
            ...
            print(
                "{}: Running multiple stress-ng stressors in "
    ...
    if test_disk.mount_filesystem(args.simulate):
        ...
        if not args.simulate:
            ...
            if not root_tmp_existed and os.path.exists("/root/tmp"):
                # Only remove if it's empty or only contains our artifacts
                try:
                    os.rmdir("/root/tmp")
                except OSError:
...
def main():
    ...
    memory_parser.add_argument(
        "-s", "--swap-size", type=int, help="swap size in GiB", default=0
    )
    memory_parser.add_argument(
        "-k",
        "--keep-swap",
    ...
    disk_parser.add_argument(
        ...
        "--base-time",
        type=int,
        help="Time for each test, in seconds " + "(default=240)",
        default=240,
    )
    disk_parser.add_argument(
        "-s",
